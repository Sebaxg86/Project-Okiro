...
type HistoryItem = { id: string; kind: ActivityKind; sourceType: string; occurredAt: string; title: string; detail: string };
...
export default async function HistoryPage({ searchParams }: { searchParams: Promise<{ status?: string; xp?: string }> }) {
  ...
  const [profileResult, workoutsResult, sleepResult, mealsResult, hydrationResult, focusResult, xpResult, weightResult] = await Promise.all([
    ...
    supabase.from("focus_sessions").select("id,started_at,duration_minutes,focus_type,objective").eq("user_id", user!.id).is("deleted_at", null).order("started_at", { ascending: false }).limit(30),
  ...
  const items: HistoryItem[] = [
    ...
    ...(sleepResult.data ?? []).map((item) => ({ id: item.id, kind: "sleep" as const, sourceType: "sleep_log", occurredAt: item.woke_up_at, title: "Sueño principal", detail: `${Math.floor(item.duration_minutes / 60)} h ${item.duration_minutes % 60} min${item.quality ? ` · calidad ${item.quality}/5` : ""}` })),
    ...
    ...(hydrationResult.data ?? []).map((item) => ({ id: item.id, kind: "hydration" as const, sourceType: "hydration_entry", occurredAt: item.occurred_at, title: `${item.amount_ml} ml de agua`, detail: "La XP se calcula sobre el total diario" })),
    ...(focusResult.data ?? []).map((item) => ({ id: item.id, kind: "focus" as const, sourceType: "focus_session", occurredAt: item.started_at, title: item.objective, detail: `${item.duration_minutes} min · ${focusLabel(item.focus_type)}` })),
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:px-10">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-display text-[10px] uppercase tracking-[0.22em] text-cyan">Trazabilidad personal</p><h1 className="mt-2 font-display text-3xl font-semibold uppercase tracking-[-0.04em]">Historial</h1><p className="mt-2 text-sm text-muted">Cada cambio de XP puede rastrearse hasta su registro.</p></div><Link href="/app/log" className="flex h-11 items-center justify-center gap-2 rounded-xl bg-accent px-5 font-display text-xs font-semibold uppercase tracking-[0.1em] text-white"><Plus size={17} />Nuevo registro</Link></header>
      ...
      <section className="system-frame panel mt-8 p-6 sm:p-8">
        <div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl border border-accent/30 bg-accent/10 text-accent"><Activity size={20} /></div><div><h2 className="font-display text-lg font-semibold uppercase">Actividades</h2><p className="text-xs text-muted">{items.length} registros activos</p></div></div>
        {items.length ? <div className="mt-6 divide-y divide-line/50">{items.map((item) => { const Icon = icons[item.kind]; const xp = xpMap.get(`${item.sourceType}:${item.id}`) ?? 0; return <article key={`${item.kind}:${item.id}`} className="grid gap-3 py-4 sm:grid-cols-[auto_1fr_auto_auto] sm:items-center"><div className="grid size-10 place-items-center rounded-xl border border-line bg-black/10 text-accent"><Icon size={18} /></div><div><h3 className="font-medium">{item.title}</h3><p className="mt-1 text-xs text-muted">{formatDateTime(item.occurredAt, timezone)} · {item.detail}</p></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${xp > 0 ? "bg-cyan/[0.08] text-cyan" : xp < 0 ? "bg-warning/[0.08] text-warning" : "bg-white/[0.04] text-muted"}`}>{xp > 0 ? "+" : ""}{xp} XP</span><ActivityActions kind={item.kind} id={item.id} idempotencyKey={randomUUID()} /></article>; })}</div> : <div className="mt-8 rounded-2xl border border-dashed border-line p-8 text-center"><Activity className="mx-auto text-muted" size={28} /><h3 className="mt-4 font-display text-sm font-semibold uppercase">Sin actividades registradas</h3><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted">Registra tu primera actividad; su XP aparecerá aquí después de calcularse en el servidor.</p><Link href="/app/log" className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-cyan">Registrar ahora <ArrowRight size={15} /></Link></div>}
      </section>

      <section className="panel mt-5 p-6 sm:p-8"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><Scale className="text-cyan" size={20} /><div><h2 className="font-display text-lg font-semibold uppercase">Mediciones de peso</h2><p className="text-xs text-muted">Separadas del sistema de XP</p></div></div><Link href="/app/profile#weight" className="text-sm font-medium text-cyan">Agregar</Link></div>{weightResult.data?.length ? <div className="mt-5 divide-y divide-line/50">{weightResult.data.map((entry) => <div key={entry.id} className="flex items-center justify-between py-3"><span className="text-sm text-muted">{formatDate(entry.measured_on)}</span><strong>{imperial ? `${(Number(entry.weight_kg) * 2.2046226218).toFixed(1)} lb` : `${Number(entry.weight_kg).toFixed(1)} kg`}</strong></div>)}</div> : <p className="mt-5 text-sm text-muted">No existen mediciones todavía.</p>}</section>
    </div>
  );
}

function noticeFor(status?: string, xp = 0) { if (status === "saved") return `Registro guardado · impacto provisional ${xp >= 0 ? "+" : ""}${xp} XP`; if (status === "updated") return `Registro actualizado · cambio provisional ${xp >= 0 ? "+" : ""}${xp} XP`; if (status === "deleted") return `Registro eliminado y XP recalculada · cambio ${xp >= 0 ? "+" : ""}${xp} XP`; if (status === "delete-error") return "No fue posible eliminar el registro."; return null; }
...
function focusLabel(value: string) { return ({ programming: "Programación", technical_study: "Estudio técnico", exercises: "Ejercicios", course: "Curso", technical_reading: "Lectura técnica", personal_project: "Proyecto personal" } as Record<string,string>)[value] ?? value; }
