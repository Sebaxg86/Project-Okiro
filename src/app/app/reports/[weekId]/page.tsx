...
export default async function WeeklyReportPage({ params }: { params: Promise<{ weekId: string }> }) {
  ...
  const [{ data: cycle }, { data: summary }, { data: transactions }] = await Promise.all([
    supabase.from("weekly_cycles").select("id,week_start,week_end,status,closed_at,goal_version_id").eq("id", weekId).eq("user_id", user!.id).maybeSingle(),
  ...
  const { data: goal } = await supabase.from("goal_versions").select("intelligence_activity_label").eq("id", cycle.goal_version_id).maybeSingle();
