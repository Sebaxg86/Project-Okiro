...
export function OnboardingForm({ initialTimezone }: { initialTimezone: string }) {
  ...
  return (
    <form action={action} className="space-y-8">
      <section>
        <h2 className="font-display text-sm font-semibold uppercase tracking-[0.12em] text-cyan">01 · Identidad</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="mb-2 block font-display text-[10px] uppercase tracking-[0.15em] text-muted">Unidades</span>
      ...
      <section>
        <h2 className="font-display text-sm font-semibold uppercase tracking-[0.12em] text-cyan">02 · Parámetros del ciclo</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <NumberField name="exerciseDaysTarget" label="Días de entrenamiento" defaultValue={5} min={0} max={7} step={0.5} suffix="días" />
          <div className="sm:col-span-2 rounded-xl border border-accent/25 bg-accent/[0.045] p-4 sm:p-5">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                <label className="block">
                  <span className="mb-2 block font-display text-[10px] uppercase tracking-[0.15em] text-muted">Disciplina mental principal</span>
          ...
          <NumberField name="intelligenceDaysTarget" label="Días de práctica" defaultValue={3} min={0} max={7} suffix="días" />
          <NumberField name="hydrationTargetMl" label="Meta diaria de hidratación" defaultValue={2500} min={250} max={10000} step={50} suffix="ml" />
          <NumberField name="expectedMainMeals" label="Comidas principales por día" defaultValue={3} min={1} max={8} suffix="al día" />
          <NumberField name="flexibleMealsPerWeek" label="Comidas flexibles por ciclo" defaultValue={2} min={0} max={21} suffix="ciclo" />
        </div>
      </section>

      <section>
        <h2 className="font-display text-sm font-semibold uppercase tracking-[0.12em] text-cyan">03 · Recuperación</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          <NumberField name="sleepMinHours" label="Descanso mínimo" defaultValue={7} min={3} max={15} step={0.5} suffix="horas" />
          <NumberField name="sleepMaxHours" label="Descanso máximo" defaultValue={9} min={3} max={15} step={0.5} suffix="horas" />
          <label className="block">
            <span className="mb-2 block font-display text-[10px] uppercase tracking-[0.15em] text-muted">Hora de recuperación</span>
      ...
      <button type="submit" disabled={pending} className="flex h-12 w-full items-center justify-center gap-2 bg-accent px-5 font-display text-xs font-semibold uppercase tracking-[0.13em] text-white shadow-[0_0_28px_rgba(139,92,255,.28)] disabled:opacity-60 [clip-path:polygon(0_9px,9px_0,100%_0,100%_calc(100%-9px),calc(100%-9px)_100%,0_100%)]">
        ...
        {pending ? "Inicializando Okiro…" : "Activar Sistema"}
      </button>
    </form>
  );
}
