...
import { Apple, BedDouble, Code2, Droplets, Dumbbell, LoaderCircle, Save } from "lucide-react";
...
const kinds = [
  ...
  { key: "focus" as const, label: "Programación", icon: Code2 },
];

export function ActivityForm({ kind, initial, idempotencyKey }: { kind: ActivityKind; initial: Record<string, string>; idempotencyKey: string }) {
    ...
    <>
      ...
      <form action={action} className="mt-8 space-y-6">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          {kind === "focus" && <FocusFields initial={initial} />}
        </div>
...
function FocusFields({ initial }: { initial: Record<string, string> }) { return <><Field name="durationMinutes" label="Duración enfocada" type="number" min="1" max="720" defaultValue={initial.durationMinutes || "50"} suffix="min" required /><Select name="focusType" label="Tipo" defaultValue={initial.focusType || "programming"} options={[["programming","Programación"],["technical_study","Estudio técnico"],["exercises","Ejercicios"],["course","Curso"],["technical_reading","Lectura técnica"],["personal_project","Proyecto personal"]]} /><div className="sm:col-span-2"><Field name="objective" label="Objetivo de la sesión" defaultValue={initial.objective} placeholder="Ej. implementar autenticación" required /></div><div className="sm:col-span-2"><Field name="projectName" label="Proyecto · opcional" defaultValue={initial.projectName} /></div></>; }
