...
import { Activity, Apple, ArrowRight, BedDouble, Brain, CalendarDays, Check, Droplets, Dumbbell, Flame, HeartPulse, Scale, ShieldCheck, Swords, Target, Trophy, Zap } from "lucide-react";
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-10">
      ...
      <section className="mt-5">
        <article className="panel p-6">
          <div className="flex items-center justify-between"><div><p className="font-display text-[10px] uppercase tracking-[0.18em] text-cyan">Misiones diarias</p><h2 className="mt-2 font-display text-xl font-semibold uppercase">{mission?.completed_required_items ?? 0}/{mission?.total_required_items ?? missionItems.length} completadas</h2></div><Flame className={mission?.status === "completed" ? "text-cyan" : "text-accent"} /></div>
          <div className="mt-6 space-y-3">{missionItems.map((item) => <div key={item.mission_type} className="flex items-center gap-3 rounded-xl border border-line/70 bg-black/10 p-3"><span className={`grid size-8 shrink-0 place-items-center rounded-lg border ${item.status === "completed" ? "border-cyan/40 bg-cyan/[0.08] text-cyan" : "border-line text-muted"}`}>{item.status === "completed" ? <Check size={15} /> : missionIcon(item.mission_type, IntelligenceIcon)}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{missionTitle(item.mission_type, item.title)}</p><p className="mt-1 text-xs text-muted">{Number(item.current_value).toFixed(0)} / {Number(item.target_value).toFixed(0)} {item.unit}</p></div></div>)}</div>
          {mission?.bonus_earned && <p className="mt-4 rounded-xl border border-cyan/25 bg-cyan/[0.05] p-3 text-sm text-cyan">Misión diaria completada · +8 XP</p>}
          <Link href="/app/missions" className="mt-5 flex items-center gap-2 text-sm font-medium text-cyan">Ver ciclo de misiones <ArrowRight size={15} /></Link>
        </article>

        <article className="panel hidden p-6">
          <div><p className="font-display text-[10px] uppercase tracking-[0.18em] text-cyan">Atributos</p><h2 className="mt-2 font-display text-xl font-semibold uppercase">Trayectoria + impulso del ciclo</h2></div>
          ...
          <Link href="/app/progress" className="mt-5 flex items-center gap-2 text-sm font-medium text-cyan">Abrir ascenso completo <ArrowRight size={15} /></Link>
        </article>
      </section>

      <section className="mt-5 grid gap-5 lg:grid-cols-[1fr_.72fr]">
        <article className="panel p-6"><div className="flex items-center justify-between"><div><p className="font-display text-[10px] uppercase tracking-[0.18em] text-cyan">Dominios del ciclo</p><h2 className="mt-2 font-display text-xl font-semibold uppercase">Estado de atributos</h2></div><Target className="text-accent" /></div>{goal && <div className="mt-6 grid gap-3 sm:grid-cols-2"><Goal label="Entrenamiento" value={`${goal.exercise_days_target} días`} xp={categoryXp("exercise")} score={live?.exercise_score} /><Goal label={intelligenceLabel} value={`${goal.programming_days_target} días`} xp={categoryXp("focus")} score={live?.programming_score} /><Goal label="Hidratación" value={`${goal.hydration_target_ml} ml`} xp={categoryXp("hydration")} score={live?.hydration_score} /><Goal label="Descanso" value={`${goal.sleep_min_minutes / 60}–${goal.sleep_max_minutes / 60} h`} xp={categoryXp("sleep")} score={live?.sleep_score} /><Goal label="Nutrición" value={`${goal.expected_main_meals} comidas`} xp={categoryXp("nutrition")} score={live?.nutrition_score} /></div>}</article>
        <article className="panel p-6"><div className="flex items-center justify-between"><div><p className="font-display text-[10px] uppercase tracking-[0.18em] text-cyan">Ciclo actual</p><h2 className="mt-2 font-display text-xl font-semibold uppercase">Activo</h2></div><CalendarDays className="text-accent" /></div>{cycle && <p className="mt-6 text-sm text-muted">{formatDate(cycle.week_start)} — {formatDate(cycle.week_end)} · {activityCount} acciones registradas</p>}<div className="mt-5 rounded-xl border border-line bg-black/10 p-4"><p className="font-display text-[9px] uppercase tracking-[0.14em] text-muted">Balance de XP</p><p className={`mt-1 text-3xl font-semibold ${provisionalXp < 0 ? "text-warning" : "text-cyan"}`}>{provisionalXp > 0 ? "+" : ""}{provisionalXp}</p></div>{latestWeight ? <p className="mt-5 flex items-center gap-2 text-xs text-muted"><Scale size={14} />Último peso: {latestWeight.weight_kg} kg</p> : null}</article>
      </section>

      <section className="mt-5 panel p-6 sm:p-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-display text-[10px] uppercase tracking-[0.18em] text-cyan">Registro rápido</p><h2 className="mt-2 font-display text-lg font-semibold uppercase">Selecciona una acción</h2></div><Link href="/app/history" className="flex items-center gap-2 text-sm font-medium text-cyan">Ver registro · {activityCount} <ArrowRight size={15} /></Link></div><div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-5"><QuickLog href="/app/log?type=workout" label="Entrenamiento" icon={Dumbbell} /><QuickLog href="/app/log?type=sleep" label="Descanso" icon={BedDouble} /><QuickLog href="/app/log?type=meal" label="Nutrición" icon={Apple} /><QuickLog href="/app/log?type=hydration" label="Hidratación" icon={Droplets} /><QuickLog href="/app/log?type=focus" label={intelligenceLabel} icon={IntelligenceIcon} /></div></section>
    </div>
  );
}
...
function MiniAttributeCard({ attribute, pending }: { attribute: AttributeRow; pending: number }) { const projected = projectAttribute(attribute.level, attribute.current_level_xp, attribute.xp_required_for_next_level, pending); const meta = attributeMeta(attribute.attribute); return <div className="rounded-xl border border-line/70 bg-black/10 p-3"><div className="flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2">{meta.icon}<span className="truncate text-xs font-medium">{meta.label}</span></div><span className="shrink-0 font-display text-[10px] text-muted">Nv. {projected.level}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#17152d]"><div className="h-full rounded-full bg-gradient-to-r from-accent to-cyan" style={{ width: `${Math.min(100, projected.xp / projected.required * 100)}%` }} /></div><p className="mt-1.5 text-[10px] text-muted">{projected.xp}/{projected.required} XP {pending > 0 && <span className="text-cyan">· +{pending}</span>}</p></div>; }
