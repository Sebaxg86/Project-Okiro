...
import { Activity, Apple, ArrowRight, BedDouble, Brain, CalendarDays, Check, Code2, Droplets, Dumbbell, Flame, HeartPulse, Plus, Scale, ShieldCheck, Swords, Target, Trophy, Zap } from "lucide-react";
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-10">
      ...
      <section className="mt-5 grid gap-5 lg:grid-cols-[1.05fr_.95fr]">
        <article className="panel p-6">
          ...
          <div className="mt-6 space-y-3">{missionItems.map((item) => <div key={item.mission_type} className="flex items-center gap-3 rounded-xl border border-line/70 bg-black/10 p-3"><span className={`grid size-8 shrink-0 place-items-center rounded-lg border ${item.status === "completed" ? "border-cyan/40 bg-cyan/[0.08] text-cyan" : "border-line text-muted"}`}>{item.status === "completed" ? <Check size={15} /> : missionIcon(item.mission_type)}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.title}</p><p className="mt-1 text-xs text-muted">{Number(item.current_value).toFixed(0)} / {Number(item.target_value).toFixed(0)} {item.unit}</p></div></div>)}</div>
...
function missionIcon(type:string){return({exercise:<Dumbbell size={14}/>,sleep:<BedDouble size={14}/>,nutrition:<Apple size={14}/>,hydration:<Droplets size={14}/>,focus:<Code2 size={14}/>} as Record<string,React.ReactNode>)[type]??<Target size={14}/>;}
