...
export function ActivityForm({ kind, initial, idempotencyKey, intelligenceType = "programming", intelligenceLabel = "Programación" }: { kind: ActivityKind; initial: Record<string, string>; idempotencyKey: string; intelligenceType?: string; intelligenceLabel?: string }) {
  ...
  const kinds = [
    ...
    { key: "sleep" as const, label: "Descanso", icon: BedDouble },
    { key: "meal" as const, label: "Nutrición", icon: Apple },
    { key: "hydration" as const, label: "Hidratación", icon: Droplets },
  ...
  return (
    <>
      ...
      <form action={action} className="mt-8 space-y-6">
        ...
        <button type="submit" disabled={pending} className="flex h-12 w-full items-center justify-center gap-2 bg-accent px-5 font-display text-xs font-semibold uppercase tracking-[0.12em] text-white shadow-[0_0_28px_rgba(139,92,255,.28)] disabled:opacity-60 [clip-path:polygon(0_9px,9px_0,100%_0,100%_calc(100%-9px),calc(100%-9px)_100%,0_100%)]">{pending ? <LoaderCircle className="animate-spin" size={17} /> : <Save size={17} />}{pending ? "Evaluando acción…" : initial.recordId ? "Actualizar registro" : "Registrar acción"}</button>
      </form>
    </>
  );
}

function WorkoutFields({ initial }: { initial: Record<string, string> }) { return <><Field name="durationMinutes" label="Duración" type="number" min="1" max="1440" defaultValue={initial.durationMinutes || "45"} suffix="min" required /><Select name="workoutType" label="Tipo de entrenamiento" defaultValue={initial.workoutType || "strength"} options={[["strength","Fuerza"],["cardio","Cardio"],["walking","Caminata"],["cycling","Ciclismo"],["swimming","Natación"],["boxing","Box"],["mma","MMA"],["sport","Deporte"],["mobility","Movilidad"],["yoga","Yoga"],["active_recovery","Recuperación activa"],["functional","Funcional"],["mixed","Mixto"],["other","Otro"]]} /><Select name="intensity" label="Intensidad" defaultValue={initial.intensity || "moderate"} options={[["light","Ligera"],["moderate","Moderada"],["intense","Intensa"]]} /><Field name="title" label="Nombre de la sesión · opcional" defaultValue={initial.title} placeholder="Fuerza de tren superior" /></>; }
function SleepFields({ initial }: { initial: Record<string, string> }) { return <><Select name="quality" label="Calidad del descanso · opcional" defaultValue={initial.quality || ""} options={[["","Sin valorar"],["1","1 · Muy baja"],["2","2 · Baja"],["3","3 · Media"],["4","4 · Buena"],["5","5 · Excelente"]]} /><Field name="interruptions" label="Interrupciones · opcional" type="number" min="0" max="50" defaultValue={initial.interruptions} /></>; }
function MealFields({ initial }: { initial: Record<string, string> }) { return <><Select name="mealType" label="Tipo de comida" defaultValue={initial.mealType || "breakfast"} options={[["breakfast","Desayuno"],["lunch","Comida"],["dinner","Cena"],["snack","Snack"],["other","Otra"]]} /><Select name="classification" label="¿Cómo encaja en tu objetivo?" defaultValue={initial.classification || "balanced"} options={[["balanced","Equilibrada"],["adequate","Adecuada"],["flexible","Flexible"],["out_of_plan","Fuera del plan"],["considerable_excess","Exceso"]]} /><div className="sm:col-span-2"><Field name="description" label="Descripción" defaultValue={initial.description} placeholder="Ej. pollo, arroz y verduras" required /></div></>; }
function HydrationFields({ initial }: { initial: Record<string, string> }) { const ref = useRef<HTMLInputElement>(null); return <div className="sm:col-span-2"><label className="block"><span className="mb-2 block font-display text-[10px] uppercase tracking-[0.15em] text-muted">Agua registrada</span><span className="relative block"><input ref={ref} name="amountMl" type="number" min="1" max="5000" defaultValue={initial.amountMl || "500"} required className={inputClass + " pr-14"} /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-muted">ml</span></span></label><div className="mt-3 grid grid-cols-3 gap-2">{[250,500,750].map((amount) => <button key={amount} type="button" onClick={() => { if (ref.current) ref.current.value = String(amount); }} className="h-10 rounded-xl border border-line text-sm text-muted hover:border-cyan/40 hover:text-cyan">{amount} ml</button>)}</div></div>; }
function FocusFields({ initial, intelligenceType, intelligenceLabel }: { initial: Record<string, string>; intelligenceType: string; intelligenceLabel: string }) { const activity = getIntelligenceActivity(initial.focusType || intelligenceType); const Icon = activity.icon; return <><input type="hidden" name="focusType" value={initial.focusType || intelligenceType} /><Field name="durationMinutes" label="Tiempo de enfoque" type="number" min="1" max="720" defaultValue={initial.durationMinutes || "50"} suffix="min" required /><div className="flex h-12 items-center gap-3 rounded-xl border border-accent/25 bg-accent/[0.06] px-4"><Icon size={18} className="text-accent" /><div><p className="text-sm font-medium">{intelligenceLabel}</p><p className="text-[10px] text-muted">Disciplina mental principal</p></div></div><div className="sm:col-span-2"><Field name="objective" label="Misión de la sesión" defaultValue={initial.objective} placeholder={`Ej. avanzar en ${intelligenceLabel.toLowerCase()}`} required /></div><div className="sm:col-span-2"><Field name="projectName" label="Proyecto · opcional" defaultValue={initial.projectName} /></div></>; }
